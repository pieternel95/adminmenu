...
RegisterNetEvent('919-admin:server:uncuffSelf', function(target)
    local src = source
    if AdminPanel.HasPermission(src, "uncuff") then
        TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "success", "<strong>SUCCESS:</strong> Uncuffed yourself.")
        TriggerEvent("qb-log:server:CreateLog", "adminactions", "Uncuff", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** Uncuffed themselves!", false)
    ...
    if AdminPanel.HasPermission(src, "setjob") then
        ...
        if targetPlayer then
            if QBCore.Shared.Jobs[targetPlayer.PlayerData.job.name].grades[grade] ~= nil then
                ...
                exports['qb-phone']:hireUser(targetPlayer.PlayerData.job.name, CID, grade)
                targetPlayer.Functions.Save()
    ...
    if Player then
        if AdminPanel.HasPermission(src, "adminmenu") then
            ...
            elseif setting == "Notifications" then
                if value == 1 or value == 0 then
                    Player.Functions.SetMetaData("adminpanel_notifications", value)
                    DebugTrace("[919-admin:server:SaveAdminMenuSetting] Set metadata adminpanel_notifications to " .. value)
                    Notifications[src] = value
                else
                    DebugTrace("[919-admin:server:SaveAdminMenuSetting] Invalid setting value")
                end
            else
