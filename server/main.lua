...
RegisterNetEvent('919-admin:server:ReportReply', function(data)
    ...
    if data.name ~= nil then
        for k, v in ipairs(QBCore.Functions.GetPlayers()) do
                ...
                local correctname = GetPlayerName(v).." ("..thePlayer.PlayerData.charinfo.firstname.." "..thePlayer.PlayerData.charinfo.lastname..")"
                if correctname == data.name then
                    citizenid = thePlayer.PlayerData.citizenid
                    TriggerClientEvent("chat:addMessage", thePlayer.PlayerData.source, {
                        template = '<div class=chat-message server">You have received a new reply to your report: <strong>{0}</strong></div>',
                        args = {data.message}
                    })
                    TriggerEvent("qb-log:server:CreateLog", "report", "Reply", "blue", GetPlayerName(src)..'    replied to : '..correctname..' Message: \n' ..data.message)
                end
        end
    end
end)
...
RegisterNetEvent('919-admin:server:ClosePanel', function()
...
OpenPanel = function(source)
    ...
    if Player then
        ...
        TriggerClientEvent('919-admin:client:OpenMenu', source, json.encode(ServerInformation.PlayerList), DarkMode[source], SeeThrough[source], Notifications[source], ServerInformation, GetConvarInt('sv_maxclients', 32), version, hasPerms)
    end
end
...
RegisterNetEvent("919-admin:server:SendReport", function(subject, info, type)
    ...
    if Player then
        ...
        TriggerEvent("qb-log:server:CreateLog", "report", "Report sent", "red", sendername..' has sent the following report: '..subject..' Message: '..info, false)
        for k, v in pairs(QBCore.Functions.GetPlayers()) do
            if AdminPanel.HasPermissionEx(v, "viewreports") then
                TriggerClientEvent("919-admin:client:ShowReportAlert", v, "NEW REPORT", sendername..": "..subject..' Report ID: '..#AdminPanel.Reports)
            end
        end
    end
end)
...
RegisterServerEvent("919-admin:server:ClaimReport", function(id)
    ...
    if AdminPanel.HasPermission(src, "claimreport") then
        if AdminPanel.Reports[tonumber(id)] then
            ...
            TriggerEvent("qb-log:server:CreateLog", "report", "Report Claimed", "yellow", "**STAFF MEMBER " .. GetPlayerName(src) .. "** claimed report ID "..id, false)
            for k, v in pairs(QBCore.Functions.GetPlayers()) do
...
RegisterServerEvent("919-admin:server:DeleteReport", function(id)
    ...
    if AdminPanel.HasPermission(src, "deletereport") then
        if AdminPanel.Reports[tonumber(id)] then
            ...
            TriggerEvent("qb-log:server:CreateLog", "report", "Report Deleted", "green", "**STAFF MEMBER " .. GetPlayerName(src) .. "** deleted report ID "..id, false)
...
RegisterNetEvent("919-admin:server:RequestViewPlayer", function(CitizenId)
    ...
    if Player then
        ...
        local lastOnlineResult = MySQL.query.await("SELECT last_updated FROM players WHERE citizenid = ?", {PlayerData.citizenid})
        ...
        local FixedPlayerData = {
            ...
            lastonline = lastOnlineResult[1].last_updated,
...
RegisterNetEvent("919-admin:server:RefreshMenu", function(silent)
    ...
    if Player then
        for k, v in pairs(QBCore.Functions.GetPlayers()) do
            ...
            local lastOnlineResult = MySQL.query.await("SELECT last_updated FROM players WHERE citizenid = ?", {thePlayer.PlayerData.citizenid})
            local amountofVehicles = MySQL.query.await('SELECT COUNT(*) as count FROM `'..Config.DB.VehiclesTable..'` WHERE citizenid = ?', {thePlayer.PlayerData.citizenid})
            table.insert(playerList,
                {
                    id = v,
                    name = GetPlayerName(v),
        ...
        TriggerClientEvent('919-admin:client:RefreshMenu', src, json.encode(playerList), silent)
    end
end)
    ...
    for k,v in ipairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player then
            if not SettingsLoaded[tonumber(v)] then
                ...
                if Player.PlayerData.metadata['adminpanel_notifications'] == nil then
                    Player.Functions.SetMetaData('adminpanel_notifications', 0)
                    Notifications[tonumber(v)] = 0
                else
                    Notifications[tonumber(v)] = Player.PlayerData.metadata['adminpanel_notifications']
                end
...
RegisterNetEvent('919-admin:server:GetPlayersForBlips', function()
    ...
    if Config.EnableNames then
        ...
        players = tempPlayers
        TriggerClientEvent('919-admin:client:Show', src, players)
    end
end)